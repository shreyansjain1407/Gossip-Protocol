...
| "3D" ->
    ...
    //         actorArray.[i] <- system.ActorOf(Props.Create(typeof<Node>, processController, 10, i+1), "ProcessController" + string i)
    let actorArray = Array3D.init gridSide gridSide gridSide (fun i j k -> system.ActorOf(Props.Create(typeof<Node>, processController, 10, i+1), "ProcessController" + string (((i*gridSide + j)*gridSide + k)*gridSide)))
    ...
    for i in [0 .. gridSide - 1] do
        for j in [0 .. gridSide - 1] do
            for k in [0 .. gridSide - 1] do
                ...
                if i = 0 && j = 0 && k = 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i+1,j,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j+1,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k+1]|]

                if i > 0 && j > 0 && k = 0 && i < gridSide - 1 && j < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k+1]|]
                if i > 0 && j = 0 && k > 0 && i < gridSide - 1 && k < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j+1,k]|]
                if i = 0 && j > 0 && k > 0 && k < gridSide - 1 && j < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i+1,j,k]|]

                if i > 0 && j = 0 && k = 0 && i < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k+1]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j+1,k]|]
                if i = 0 && j = 0 && k > 0 && k < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j+1,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i+1,j,k]|]
                if i = 0 && j > 0 && k = 0 && j < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i+1,j,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k+1]|]
                
                if i > 0 && j > 0 && k > 0 && i < gridSide - 1 && j < gridSide - 1 && k < gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i+1,j,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i-1,j,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j+1,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j-1,k]|]
                ...
                if i < gridSide - 1 && j = gridSide - 1 && k = gridSide - 1 && i > 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k-1]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j-1,k]|]
                if i = gridSide - 1 && j = gridSide - 1 && k < gridSide - 1 && k > 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j-1,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i-1,j,k]|]
                if i = gridSide - 1 && j < gridSide - 1 && k = gridSide - 1 && j > 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i-1,j,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k-1]|]

                if i < gridSide - 1 && j < gridSide - 1 && k = gridSide - 1 && i < 0 && j < 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k-1]|]
                if i < gridSide - 1 && j = gridSide - 1 && k < gridSide - 1 && i > 0 && k > 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j-1,k]|]
                if i = gridSide - 1 && j < gridSide - 1 && k < gridSide - 1 && k > 0 && j > 0 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i-1,j,k]|]

                if i = gridSide - 1 && j = gridSide - 1 && k = gridSide - 1 then
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i-1,j,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j-1,k]|]
                    neighbourArr <- Array.append neighbourArr[|actorArray.[i,j,k-1]|]
                    // printfn "%i  Neighbours" neighbourArr.Length
                    actorArray.[i,j,k] <! Initialization(neighbourArr)
    //Loop to initialize the neighbours of spawned actors in this case all are neighnours
    ...
    if algo = "gossip" then
        ...
        //Initializing the first actor in the chain reaction
        actorArray.[Random().Next(0, gridSide - 1),Random().Next(0, gridSide - 1),Random().Next(0, gridSide - 1)] <! Rumor("This is some top secret info, do not disclose")
    else if algo = "pushsum" then
        stopWatch.Start()
        processController <! SetStart(stopWatch.ElapsedMilliseconds)
        actorArray.[Random().Next(0, gridSide - 1),Random().Next(0, gridSide - 1),Random().Next(0, gridSide - 1)] <! PushSumInit
